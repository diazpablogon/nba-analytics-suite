"""Placeholder downloader for team dashboard endpoints."""
...
list_team_ids = tools.list_team_ids
output_root = tools.output_root

LOGGER = logging.getLogger("01g_get_team_dashboards")
logging.basicConfig(level=logging.INFO, format="%(levelname)s: %(message)s")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Download team dashboard endpoints (placeholder).")
...
def main() -> int:
    ...
    teams = list_team_ids()
    LOGGER.info("Placeholder run for %d teams.", len(teams))
    ...
    for season in seasons:
        for season_type in season_types(args.include_playoffs):
            for team_id in teams:
                LOGGER.info(
                    "TODO: Implement team dashboard downloads for team %s (%s %s) -> %s",
                    team_id,
                    season,
                    season_type,
                    output_root() / "team_dashboard",
                )
